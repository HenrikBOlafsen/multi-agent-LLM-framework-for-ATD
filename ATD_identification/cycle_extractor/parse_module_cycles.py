...
# ---- cycle helpers (deterministic, shortest-first) ----
K_CYCLES_PER_SCC = 5
DEDUP_IGNORE_DIRECTION = True

def canonicalize_cycle(nodes: List[str]):
    if not nodes: return tuple()
    cyc = list(nodes); n = len(cyc)
    i_min = min(range(n), key=lambda i: cyc[i])
    fwd = tuple(cyc[i_min:] + cyc[:i_min])
    if not DEDUP_IGNORE_DIRECTION: return fwd
    rc = list(reversed(cyc)); j_min = min(range(n), key=lambda i: rc[i])
...
# ---- JSON shapers (matches your example format) ----
