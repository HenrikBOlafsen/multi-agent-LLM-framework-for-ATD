...
def _exists_with_exact_case(p: str) -> bool:
    """
    Return True only if the path exists AND every segment matches the on-disk
    name with identical casing. Works on case-insensitive filesystems.
    """
    try:
        ...
        for seg in parts:
            try:
                ...
                return False
            # must match exactly one entry with identical case
...
def _module_to_file(mod: str, paths: Dict[str, str], top_pkg: str) -> str | None:
    """
    Prefer pydeps path (only if it exists with exact case),
    else accept path-like (only if it exists with exact case),
    else use import system for our top_pkg namespace (also exact-case).
    """
    p = paths.get(mod)
    if p:
        if _exists_with_exact_case(p):
            return p
        return None
    if _is_path_like(mod):
        if _exists_with_exact_case(mod):
            return mod
        return None
    if top_pkg and (mod == top_pkg or mod.startswith(top_pkg + ".")):
        specp = _safe_find_spec(mod)
        if specp and _exists_with_exact_case(specp):
            return specp
    return None
...
def build_graph_from_pydeps(pydeps_json: str, repo_root: str) -> nx.DiGraph:
    ...
    # Map module -> abs path (filtered to repo; require real files with exact case)
    ...
    for mod in imports.keys():
        ...
        if not _exists_with_exact_case(rp):
            continue
        if rp.startswith(root) and _exists_with_exact_case(rp):
            mod_abs[mod] = rp
