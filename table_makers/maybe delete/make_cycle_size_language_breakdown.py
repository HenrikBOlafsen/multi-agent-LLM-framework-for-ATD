...
def _language_sort_rank(language: str) -> int:
    normalized = str(language).strip().lower()
    if normalized == "python":
        return 0
    if normalized in {"csharp", "c#", "cs", "dotnet", ".net"}:
...
def _cycle_size_sort_rank(cycle_size_bin: str) -> int:
    return {
        "2--3": 0,
        "4--6": 1,
        "7--8": 2,
...
def _mode_sort_rank(mode_id: str, mode_ids: list[str]) -> int:
    try:
        return mode_ids.index(mode_id)
    except ValueError:
        return 999


def build_cycle_size_language_breakdown_rows(all_runs_csv_path: Path) -> pd.DataFrame:
    ...
    languages = list(df["language"].dropna().drop_duplicates())

    rows: List[Dict[str, object]] = []

    for language in languages:
        language_df = df[df["language"] == language]
        display_language = _display_language(str(language))

        for cycle_size_bin in cycle_size_bins:
            bin_df = language_df[language_df["cycle_size_bin"] == cycle_size_bin]
    ...
    out["_language_sort"] = out["language"].apply(_language_sort_rank)
    out["_cycle_size_sort"] = out["cycle_size_bin"].apply(_cycle_size_sort_rank)
    out["_mode_sort"] = out["mode_id"].apply(lambda x: _mode_sort_rank(str(x), mode_ids))

    out = out.sort_values(
        by=["_language_sort", "_cycle_size_sort", "_mode_sort"],
        kind="stable",
    ).drop(columns=["_language_sort", "_cycle_size_sort", "_mode_sort"])

    return out.reset_index(drop=True)


def write_cycle_size_language_breakdown_csv(all_runs_csv_path: Path, outdir: Path) -> Path:
    df = build_cycle_size_language_breakdown_rows(all_runs_csv_path)
    out_path = outdir / "cycle_size_language_breakdown.csv"
