...
SYNTHESIZER_MIN_OUTPUT_TOKENS_RESERVED = 3334
...
Write a natural-language memo for a later refactoring agent. Use no forced headings unless they genuinely help.
...
""".rstrip() + "\n"
    ...
    if has_aux:
        ...
        need = estimate_tokens_from_text(normalized_aux_text) + estimate_tokens_from_chars(
            prompt_block_wrapper_len(aux_block_id)
        )
        budget_item_needs_tokens.append(int(need))
    ...
    for block_id, text, allocated_tokens in zip(
        block_ids,
        (normalized_edge_reports + ([normalized_aux_text] if has_aux else [])),
        budget_item_allocations_tokens,
    ):
