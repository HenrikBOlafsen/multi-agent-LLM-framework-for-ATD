...
Break the cycle by removing at least one edge while ensuring no new cycles are introduced and behavior remains unchanged.

Important:
- My ATD metric treats any module/type reference as a dependency, so making imports dynamic or lazy is not sufficient unless the dependency no longer counts under the rules above.
- We care about architecture (static coupling), not runtime import order.
- Prefer solutions that reduce the structural coupling between the modules rather than introducing artificial indirection or temporary wrappers.
...
- The cycle is resolved through a meaningful architectural change (for example clearer module responsibilities or reduced coupling).
...
- If you introduce a new file, do not just make the cycle longer (for example, A->C->B->A).
